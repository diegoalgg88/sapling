...
**Argument:** `$ARGUMENTS` — optional: a label or area to focus on (e.g., `cli`, `context`, `tools`). If empty, analyze everything.
...
### c. Complexity
- Is the change localized or does it cut across multiple subsystems (CLI, tools, context, client)?
